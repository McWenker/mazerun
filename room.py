...
class Room(object):
	...
	def __init__(self):
		...
		for t in xrange(const.TREASURES):
			while 1:
				col = randint(0,const.COLUMNS-1)
				row = randint(0,const.ROWS-1)
				treasure = Treasure((row*const.TILE_SIZE),(col*const.TILE_SIZE))
				if not self.walls[row][col] and self.floor[row][col]:
					self.treasure[row][col] = treasure
					self.treasure_list.add(treasure)
					break
		
		for m in xrange(const.ENEMIES):
			while 1:
				col = randint(0,const.COLUMNS-1)
				row = randint(0,const.ROWS-1)
				enemy = Enemy()
				enemy.rect.left = row*const.TILE_SIZE
				enemy.rect.top = col*const.TILE_SIZE
	...
	def fill_map(self):
		for i in range(const.ROWS):
			for j in range(const.COLUMNS):
				wall = Wall(i*const.TILE_SIZE,j*const.TILE_SIZE)
	...
	def get_chambers(self):
		...
		chambers = 1
		keep_going = 50
		while chambers <= const.MAX_CHAMBERS and keep_going:
			height = randint(4,10)
			length = randint(4,10)
			x = randint(0,const.COLUMNS-1)
			y = randint(0,const.ROWS)
		...
		for chamber in self.chamber_list:
			...
			for x in xrange(start,end):
				...
				for w in self.wall_list:
					if w.rect.left == x*const.TILE_SIZE and w.rect.top == (
							chamber.door[1]*const.TILE_SIZE):
			...
			for y in xrange(start,end):
				...
				for w in self.wall_list:
					if w.rect.left == next.door[0]*const.TILE_SIZE and w.rect.top == (
							y*const.TILE_SIZE):
						self.wall_list.remove(w)
	...
	def check_chamber(self, coord, height, length):
		for i in xrange(0,height):
			for j in xrange(0,length):
				if coord[1]+i > const.COLUMNS-1:
					return False
				if coord[0]+j > const.ROWS-1:
					return False
	...
	def create_chamber(self, chamber):
		...
		for i in xrange(0,chamber.length):
			...
			snd = chamber.start[1]
			wall = Wall(fst*const.TILE_SIZE,snd*const.TILE_SIZE)
	...
	def make_random_door(self, chamber):
		while 1:
			...
			if door:
				...
				for w in self.wall_list:
					if w.rect.left == coord[0]*const.TILE_SIZE and w.rect.top == (
							coord[1]*const.TILE_SIZE):
						self.wall_list.remove(w)
	...
	def is_block_empty(self, row, col):
		...
		and not (self.player[0]/const.TILE_SIZE,self.player[1]/const.TILE_SIZE) == (row,col):
			return True
		else:
			return False
			
	def has_wall(self, row, col):
		row = row/const.TILE_SIZE
		col = col/const.TILE_SIZE
